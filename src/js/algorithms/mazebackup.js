...
const divide = function(initXPos, initYPos, endXPos, endYPos) {
  ...
  /* add walls depending on the orientation */
  if (direction === 'vertical') { // width > height
    ...
    // const wallY = Math.floor(Math.random() * ( (endYPos - 1) - (initYPos + 1) + 1 )) + (initYPos + 1); //Math.floor(Math.random() * (max - min + 1)) + min;
    let wallY = 1;
    while (wallY % 2 != 0) {
